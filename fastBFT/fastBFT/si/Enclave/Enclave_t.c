...
static sgx_status_t SGX_CDECL sgx_update_counter(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_update_counter_t));
	ms_update_counter_t* ms = SGX_CAST(ms_update_counter_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	uint8_t* _tmp_sc = ms->ms_sc;
	size_t _len_sc = 16;
	uint8_t* _in_sc = NULL;
	sgx_ec256_signature_t* _tmp_up_signature = ms->ms_up_signature;
	size_t _len_up_signature = 64;
	sgx_ec256_signature_t* _in_up_signature = NULL;
	uint8_t* _tmp_hc1 = ms->ms_hc1;
	size_t _len_hc1 = 32;
	uint8_t* _in_hc1 = NULL;
	uint8_t* _tmp_result = ms->ms_result;
	size_t _len_result = 35;
	uint8_t* _in_result = NULL;

	CHECK_UNIQUE_POINTER(_tmp_sc, _len_sc);
	CHECK_UNIQUE_POINTER(_tmp_up_signature, _len_up_signature);
	CHECK_UNIQUE_POINTER(_tmp_hc1, _len_hc1);
	CHECK_UNIQUE_POINTER(_tmp_result, _len_result);

	if (_tmp_sc != NULL && _len_sc != 0) {
		_in_sc = (uint8_t*)malloc(_len_sc);
		if (_in_sc == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		memcpy(_in_sc, _tmp_sc, _len_sc);
	}
	if (_tmp_up_signature != NULL && _len_up_signature != 0) {
		_in_up_signature = (sgx_ec256_signature_t*)malloc(_len_up_signature);
		if (_in_up_signature == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		memcpy(_in_up_signature, _tmp_up_signature, _len_up_signature);
	}
	if (_tmp_hc1 != NULL && _len_hc1 != 0) {
		_in_hc1 = (uint8_t*)malloc(_len_hc1);
		if (_in_hc1 == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		memcpy(_in_hc1, _tmp_hc1, _len_hc1);
	}
	if (_tmp_result != NULL && _len_result != 0) {
		if ((_in_result = (uint8_t*)malloc(_len_result)) == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
	...
	update_counter(_in_sc, _in_up_signature, _in_hc1, _in_result);
err:
	if (_in_sc) free(_in_sc);
	if (_in_up_signature) free(_in_up_signature);
	if (_in_hc1) free(_in_hc1);
	if (_in_result) {
		memcpy(_tmp_result, _in_result, _len_result);
		free(_in_result);
	}
...
SGX_EXTERNC const struct {
	...
	5,
	{
		...
		{(void*)(uintptr_t)sgx_update_counter, 0},
